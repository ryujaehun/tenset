...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default='arm',choices=['arm','plat','e5','epyc','k80','t4'])
    ...
    if args.wandb:
        if args.maml:
            wandb.init(name=f'META_{args.models}_{args.loss}_{args.dataset}',project=f"BASELINE_PRETRAIN_W/O", tags=[f"META",f'{args.models}'])
        elif args.models in ['xgb','lgbm','random']:
            wandb.init(name=f'{args.models}',project=f"BASELINE_PRETRAIN_W/O", tags=[f"BASELINE",f'{args.models}'])
        else:
            wandb.init(name=f'{args.models}_{args.loss}_{args.dataset}',project=f"BASELINE_PRETRAIN_W/O", tags=[f"{args.models}",f"{args.loss}"])
        wandb.config.update(args)
    else:
        wandb = None
    args.save = f'BASELINE_PRETRAIN_{args.models}_{args.loss}_{args.dataset}'
    if args.maml:
        args.save += f'_maml'
    # Setup random seed and logging
    dataset = ['arm','plat','e5','epyc','k80','t4']
    dataset.remove(args.dataset)
