...
def eval_cost_model_on_network(model, network_key, target, top_ks,args):
    ...
    if args.maml:
        ...
        eval_res = evaluate_model(model, dataset,.7)
...
if __name__ == "__main__":
    ...
    if args.wandb:
        if args.maml:
            wandb.init(name=f'MAML_{args.models}_{args.loss}',project=f"NEW_EXP_2", tags=[f"META",f'{args.models}'])
        elif args.models in ['xgb','lgbm','random']:
            wandb.init(name=f'{args.models}',project=f"NEW_EXP_2", tags=[f"BASELINE",f'{args.models}'])
        else:
            wandb.init(name=f'{args.models}_{args.loss}',project=f"NEW_EXP_2", tags=[f"{args.models}",f"{args.loss}"])
        wandb.config.update(args)
    else:
        wandb = None
    args.save = f'EXP_2_{args.models}_{args.loss}'
    if args.maml:
        args.save += f'_maml'
