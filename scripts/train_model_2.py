...
if __name__ == "__main__":
    ...
    if args.wandb:
        if args.maml:
            wandb.init(name=f'MAML_{args.models}_{args.loss}',project=f"NEW_EXP_7", tags=[f"META",f'{args.models}'])
        elif args.models in ['xgb','lgbm','random']:
            wandb.init(name=f'{args.models}',project=f"NEW_EXP_7", tags=[f"BASELINE",f'{args.models}'])
        else:
            wandb.init(name=f'{args.models}_{args.loss}',project=f"NEW_EXP_7", tags=[f"{args.models}",f"{args.loss}"])
