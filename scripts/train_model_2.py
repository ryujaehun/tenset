...
if __name__ == "__main__":
    ...
    if args.wandb:
        if args.maml:
            wandb.init(name=f'META_{args.models}_{args.loss}_TRAIN_{_data}',project=f"FINAL_PRETRAIN_TRAIN_0925", tags=[f"META",f'{args.models}'])
        elif args.models in ['xgb','lgbm','random']:
            wandb.init(name=f'{args.models}_TRAIN_{_data}',project=f"FINAL_PRETRAIN", tags=[f"BASELINE",f'{args.models}'])
        else:
            wandb.init(name=f'{args.models}_{args.loss}_TRAIN_{_data}',project=f"FINAL_PRETRAIN_TRAIN_0925", tags=[f"{args.models}",f"{args.loss}"])
