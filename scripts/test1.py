...
model = ['mlp','oneshot']
device = ['arm','plat','e5','epyc','k80','t4']
...
class Worker(threading.Thread):
    ...
    def run(self):
        ...
        if self.maml:
         ...
         --dataset {self.dataset} --maml --wandb   --use-gpu --loss {self.loss} --models {self.model}  >& log/{today}/MAML_PRETRAIN_{self.dataset}_{self.model}_{self.loss}.log"
        else:
            text = f"docker run --ipc=host -it --gpus '\"device={idx%count}\"' --cpus 8 --rm  -v /home/jaehun/tenset/2080:/build -v /home/jaehun/tenset:/root/tvm -v /home/jaehun/tenset:/root test:latest python3 /root/scripts/train_model_1.py \
            --dataset {self.dataset} --wandb   --use-gpu --loss {self.loss} --models {self.model}  >& log/{today}/PRETRAIN_{self.dataset}_{self.model}_{self.loss}.log"
...
for _loss in loss:
    for _model in model:
        for _dataset in device:
            thread = Worker(_loss,_model,_dataset)
